...
<!-- ========== HERO CARD ========== -->
<div
	class="card mb-4 border border-orange-500/30 bg-gradient-to-br from-orange-900/20 to-red-900/20 backdrop-blur-md"
>
			...
			<div class="shrink-0">
				{#if data.restaurant.doenerImage}
					<img
						...
						class="h-56 w-full rounded-xl border-2 border-orange-500/30 object-cover shadow-lg lg:w-56"
					/>
				{:else}
					<div
						class="flex h-56 w-full items-center justify-center rounded-xl border-2 border-orange-500/30 bg-gradient-to-br from-orange-600/40 to-red-600/40 lg:w-56"
					>
			...
			<div class="flex-1 space-y-3">
				...
				<div class="flex items-start justify-between gap-3">
					<div>
						...
						<a
							...
							class="mt-1 flex w-fit items-center gap-1.5 text-orange-300/80 transition-colors hover:text-orange-200"
						>
					...
					<button
						...
						class="btn btn-circle {isFavorite
							? 'btn-warning'
				...
				<div class="flex items-center gap-3">
					<div class="flex items-center gap-1.5 rounded-xl border px-3 py-1.5 {rb(overall)}">
						...
						<span class="text-xl font-bold {rc(overall)}">
							{overall > 0 ? overall.toFixed(1) : 'N/A'}
						</span>
					</div>
					<div class="text-sm text-orange-300/80">
						<span class="text-base font-semibold text-white"
							>{data.restaurant.reviewCount}</span
						>
						review{data.restaurant.reviewCount !== 1 ? 's' : ''} ·
						<span class={rc(overall)}>{rl(overall)}</span>
					</div>
				</div>
				...
				{#if a}
					<div class="flex flex-wrap gap-2">
						{#if a.avgPrice != null}
							<div
								class="rounded-lg border border-green-500/30 bg-green-500/10 px-2.5 py-1 text-sm"
							>
				...
				{:else if data.userHasReviewed}
					<div class="inline-flex items-center gap-1.5 text-sm text-green-300">
						<span>✓</span> You've reviewed this döner
					</div>
				{/if}
			</div>
		</div>
	...
	<div class="mb-4 space-y-3">
		...
		<div class="rounded-xl border border-white/5 bg-slate-800/50 p-4">
			...
			<div class="space-y-2.5">
				{#each [{ l: '🥩 Meat', v: a.avgMeat }, { l: '🍞 Bread', v: a.avgBread }, { l: '🥬 Veggies', v: a.avgVeggies }, { l: '🫗 Sauce', v: a.avgSauce }, { l: '⭐ Flavor', v: a.avgFlavor }, { l: '🧹 Hygiene', v: a.avgCleanliness }] as cat}
					<div class="flex items-center gap-3">
						<span class="w-24 shrink-0 text-sm text-gray-300">{cat.l}</span>
						<div class="relative h-3 flex-1 overflow-hidden rounded-full bg-slate-700">
							<div
								class="absolute inset-y-0 left-0 rounded-full transition-all duration-500 {cat.v != null && cat.v >= 3.5
									? 'bg-green-500'
									: cat.v != null && cat.v >= 2.5
										? 'bg-blue-500'
										: cat.v != null && cat.v >= 1.5
											? 'bg-yellow-500'
											: 'bg-orange-500'}"
								style="width: {barW(cat.v)}"
							></div>
						</div>
						<span class="w-8 text-right text-sm font-semibold {rc(cat.v ?? 0)}">
							{cat.v != null ? cat.v.toFixed(1) : '–'}
						</span>
					</div>
				{/each}
			</div>
		...
		<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
			<!-- Meat details -->
			<div class="rounded-xl border border-white/5 bg-slate-800/50 p-4">
				<h3 class="mb-2.5 text-sm font-semibold tracking-wide text-gray-400 uppercase">
					🥩 Meat Profile
				</h3>
				<div class="space-y-2">
						...
						<div class="flex items-center justify-between">
							...
							<div class="flex items-center gap-1.5">
								<div class="flex gap-0.5">
									{#each [1, 2, 3, 4] as dot}
										<div
											class="size-2 rounded-full {(row.v ?? 0) >= dot
												? 'bg-orange-400'
												: 'bg-slate-600'}"
										></div>
									{/each}
								...
								<span class="w-6 text-right text-xs text-gray-500"
									>{row.v != null ? row.v.toFixed(1) : '–'}</span
								>
							</div>
						</div>
					{/each}
					...
					{#if a.shapes.length > 0}
						<div class="flex items-center justify-between">
							...
							<div class="flex gap-1">
								{#each a.shapes as s}
									<span class="badge badge-xs border-amber-400/30 bg-amber-500/15 text-amber-200"
										>{s.label}
										<span class="opacity-50">{s.pct}%</span></span
									>
								{/each}
							</div>
						</div>
				...
				<div class="rounded-xl border border-white/5 bg-slate-800/50 p-4">
					...
					<div class="flex flex-wrap gap-1.5">
						{#each a.topVeggies as v}
							<span class="badge badge-sm border-green-400/30 bg-green-500/15 text-green-200">
								{veggieEmoji[v.label] || '🥗'} {v.label}
								<span class="ml-0.5 opacity-50">{v.pct}%</span>
							</span>
						{/each}
					</div>
				</div>
				...
				<div class="rounded-xl border border-white/5 bg-slate-800/50 p-4">
					...
					<div class="flex flex-wrap gap-1.5">
						{#each a.topSauces as s}
							<span class="badge badge-sm border-blue-300/30 bg-blue-400/15 text-blue-200">
								{sauceEmoji[s.label] || '🫗'} {s.label}
								<span class="ml-0.5 opacity-50">{s.pct}%</span>
							</span>
						{/each}
					</div>
				</div>
	...
	>
		<div class="card-body items-center py-10">
			...
			{#if data.user}
				<a
					href="/doener/{data.restaurant.id}/review"
					class="btn btn-sm mt-3 border-0 bg-gradient-to-r from-orange-600 to-red-600 text-white"
				>
					<FluentAdd24Regular class="size-4" />
					Write a Review
				</a>
					...
					<div class="rounded-xl border border-white/5 bg-slate-800/40 p-4">
						...
						<div class="mb-2.5 flex items-center justify-between">
							...
							<div class="flex items-center gap-1 rounded-md border px-1.5 py-0.5 {rb(review.overallRating)}">
								...
								<span class="text-xs font-bold {rc(review.overallRating)}"
							...
							{#each [{ l: 'Meat', v: review.meatRating }, { l: 'Bread', v: review.breadRating }, { l: 'Veggies', v: review.veggiesRating }, { l: 'Sauce', v: review.sauceRating }, { l: 'Flavor', v: review.overallFlavorRating }, { l: 'Hygiene', v: review.cleanlinessRating }] as cat}
								<div class="flex items-center gap-2">
									...
									<div class="relative h-1.5 flex-1 overflow-hidden rounded-full bg-slate-700">
										<div
											class="absolute inset-y-0 left-0 rounded-full {cat.v != null && cat.v >= 3.5
												? 'bg-green-500'
												: cat.v != null && cat.v >= 2.5
													? 'bg-blue-500'
													: cat.v != null && cat.v >= 1.5
									...
									<span class="w-4 text-right text-[10px] text-gray-500"
										>{cat.v ?? '–'}</span
									>
								</div>
							{/each}
						...
						<div class="mb-2 flex flex-wrap gap-1">
							{#if review.meatChicken}<span
									class="badge badge-xs border-red-400/30 bg-red-500/15 text-red-200"
									>🐔 Chicken</span
								>{/if}
							{#if review.meatBeef}<span
									class="badge badge-xs border-red-400/30 bg-red-500/15 text-red-200"
									>🐄 Beef</span
								>{/if}
							{#if review.meatLamb}<span
									class="badge badge-xs border-amber-400/30 bg-amber-500/15 text-amber-200"
									>🐑 Lamb</span
								>{/if}
							{#if review.meatStyle}<span
									class="badge badge-xs border-orange-400/30 bg-orange-500/15 text-orange-200"
									>🥩 {review.meatStyle === 'minced' ? 'Minced' : 'Layered'}</span
								>{/if}
							{#if review.breadShape}<span
									class="badge badge-xs border-amber-400/30 bg-amber-500/15 text-amber-200"
									>🍞 {review.breadShape}</span
								>{/if}
							{#if review.breadSesameSeeds}<span
									class="badge badge-xs border-amber-400/30 bg-amber-500/15 text-amber-200"
									>🌰 Sesame</span
								>{/if}
							{#if review.hasHerbalSauce}<span
									class="badge badge-xs border-green-300/30 bg-green-400/15 text-green-200"
									>🌿 Herbal</span
								>{/if}
							{#if review.hasYoghurtSauce}<span
									class="badge badge-xs border-blue-300/30 bg-blue-400/15 text-blue-200"
									>🥛 Yoghurt</span
								>{/if}
							{#if review.hasGarlicSauce}<span
									class="badge badge-xs border-purple-300/30 bg-purple-400/15 text-purple-200"
									>🧄 Garlic</span
								>{/if}
							{#if review.hasCocktailSauce}<span
									class="badge badge-xs border-pink-300/30 bg-pink-400/15 text-pink-200"
									>🍹 Cocktail</span
								>{/if}
							{#if review.hasSpicySauce}<span
									class="badge badge-xs border-red-300/30 bg-red-400/15 text-red-200"
									>🌶️ Spicy</span
								>{/if}
							{#if review.doenerSize}<span
									class="badge badge-xs border-slate-400/30 bg-slate-500/15 text-slate-300"
									>📏 {review.doenerSize}</span
								>{/if}
							{#if review.price != null}<span
									class="badge badge-xs border-green-400/30 bg-green-500/15 text-green-200"
									>💰 €{review.price.toFixed(2)}</span
								>{/if}
						</div>

						<!-- Description -->
						{#if review.description}
							<p class="rounded-lg bg-slate-900/40 p-2.5 text-sm leading-relaxed text-gray-300 italic">
								"{review.description}"
							</p>
						{/if}
					</div>
